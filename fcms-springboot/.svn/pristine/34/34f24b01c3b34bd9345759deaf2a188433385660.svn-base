...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		findElements: function() {
			...
			self.el.others = self.box.querySelector(classSelector('indexed-list-others'));
//			console.log(self.el.others.style.width );
		...
		caleLayout: function() {
			...
			if(self.el.others != null){// others 指的是除了demo页面的search和list的高度 其他的数据
				withoutSearchHeight = (self.box.offsetHeight - self.el.search.offsetHeight - self.el.others.offsetHeight - 10) + 'px';
			}
			self.el.bar.style.height = withoutSearchHeight;
			self.el.inner.style.height = withoutSearchHeight;
//			alert(withoutSearchHeight);
