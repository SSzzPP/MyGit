(function($, owner) {
	...
	owner._downloadUpdateFile = function(downloadUrl){
	    ...
	    console.log("download-path:"+serverPath+downloadUrl);
	    var task = plus.downloader.createDownload(serverPath+downloadUrl, {filename:"_doc/update/",retry: 1}, function(d,status){
	        if ( status == 200 ) { 
