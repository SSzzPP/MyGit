...
$(function() {
    ...
    videoDatagrid = $('#videoDatagrid').datagrid({
    	url: '${dynamicURL}/admin/video/list?sort=gmtCreate&order=desc',
...
function add() {
	parent._layer({
        url: '${dynamicURL}/admin/video/add',
        title: "新增视频",
        width:'600px',
        height:'600px',
        btn: ['保存', '取消']
    },
    {
        yes: function(index, layero) {
            ...
            $.submit({
	            url: "${dynamicURL}/admin/video/saveInfo",
	            formTg:frm,
	            butObj : bObj,
	            cache: false,
...
/*修改*/
function edit() {
	var row = videoDatagrid.datagrid("getSelected");
	if(row){
		parent._layer({
	        url: '${dynamicURL}/admin/video/add?id='+row.id,
	        title: "修改视频",
	        width:'600px',
	        height:'600px',
	        btn: ['保存', '取消']
...
function del() {
    ...
    if (row) {
        $.submit({
            url: "${dynamicURL}/admin/video/deleteInfo",
            data:{
                "ids": row.id
            },
            butObj : null,
