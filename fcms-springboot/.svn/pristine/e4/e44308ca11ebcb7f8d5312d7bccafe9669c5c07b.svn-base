...
<body>
    ...
    <script>
        ...
        $(function(){
            ...
            loadBusinessArea();
            loadAdminArea();
            loadVenue(business,trative,typeId,0);
            loadLike();
        })
        function venueType(){
        	$.submit({
				url: dynamicURL+"/front/dict/list?search_EQ_type=SPORT_TYPE",
				success: function(result){
				    if(result.rows.length>0){
				    	var str= '';
				    	$.each(result.rows,function(index,value){
				    		str+=' <a onclick="loadVenue('+business+','+trative+','+value.id+',0)" data-cat-id="'+value.id+'" data-click-name="select_reg_hot_click" data-click-title="'+value.name+'"><span>'+value.name+'</span></a>';
				    	});
				    	$("#venueType").append(str);
				    }
				}
			})
        } 
       function loadBusinessArea(){
        	$.submit({
				url: dynamicURL+"/front/dict/list?search_EQ_type=BUSINESS_AREA",
				success: function(result){
				    if(result.rows.length>0){
				    	var str= '';
				    	$.each(result.rows,function(index,value){
				    		str+=' <a onclick="loadVenue('+value.id+','+trative+','+typeId+',0)" data-cat-id="'+value.id+'"data-cat-id="'+value.id+'" data-click-name="select_reg_hot_click" data-click-title="'+value.name+'"><span>'+value.name+'</span></a>';
				    	});
				    	$("#bussi-nav").append(str);
				    }
				}
			})
        }
        function loadAdminArea(){
        	$.submit({
				url: dynamicURL+"/front/dict/list?search_EQ_type=ADMINISTRATE_AREA",
				success: function(result){
				    if(result.rows.length>0){
				    	var str= '';
				    	$.each(result.rows,function(index,value){
				    		str+=' <a onclick="loadVenue('+business+','+value.id+','+typeId+',0)" data-cat-id="'+value.id+'" data-cat-id="'+value.id+'" data-click-name="select_reg_hot_click" data-click-title="'+value.name+'"><span>'+value.name+'</span></a>';
				    	});
				    	$("#region-nav").append(str);
				    }
				}
			})
        }
        function loadVenue(business,trative,typeId,pageNum){
			var params = {
				'page':pageNum+1,
				'rows':pageSize
			};
        	...
        	$.submit({
				...
				success: function(result){
				    ...
				    if(result.rows.length>0){
				    	var str= '';
				    	$.each(result.rows,function(index,value){
				    		 str+='<li onclick="venueDetails('+value.id+')">';
				    		 str+='<div class="pic">';
				    		 str+='<a href="javascript:void(0)" onclick="venueDetails('+value.id+')">';
				    		...
				    		// str+='        </a>';
			    			 ...
			    			 str+='条点评</a>'; */
			    			 str+=' <em class="sep">|</em>';
			    			 str+=' <a class="mean-price">人均<b>￥'+value.perCapita+'</b></a>';
			    			 str+='</div>';
			    			 str+=' <div class="tag-addr">';
			    			 str+='<a href=""><span>'+value.name+'</span></a>';
			    			 str+='<em class="sep">|</em>';
			    			 str+='      <a href=""><span>'+value.businessHours+'</span></a>';
			    			 str+='<em class="sep">|</em>';
			    			 str+=' <span class="addr">'+value.location+'</span>';
			    			 str+='  </div>';
			    			 str+='</div>';
        ...
        function pageselectCallback(page_index, jq){
            ...
            text = "-";
            page = page_index;
            loadVenue(business,trative,typeId,page_index);
            return false;
        }
    </script>
</body>
