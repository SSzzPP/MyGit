...
<!--场馆评价页面  -->
	...
	<body>
		...
		<div class="mui-content">
			<form action="javascript:void(0)">
				...
				<div class="mui-content-padded">
					...
					<div class="icons service-icon mui-inline" style="margin-left: 6px;">
						<i data-index="1" class="mui-icon mui-icon-star"></i>
						<i data-index="2" class="mui-icon mui-icon-star"></i>
						<i data-index="3" class="mui-icon mui-icon-star"></i>
						<i data-index="4" class="mui-icon mui-icon-star"></i>
		...
		<script type="text/javascript">
			...
			mui('.service-icon').on('tap','i',function(){
			  	var index = parseInt(this.getAttribute("data-index"));
			  	var parent = this.parentNode;
			  	var children = parent.children;
			  	if(this.classList.contains("mui-icon-star")){
					...
					if(e.index==1){
						...
						$('#submit').attr('disabled',true);
						  param.orderUuid = orderUuid;
					        ...
					        param.environmentStar = environmentIndex;
					        param.serviceStar = serviceIndex;
					        topUtil.web_query("/mobile/venue/vealuate/saveVealuate",param,function(data){
					        	if(data.success){
