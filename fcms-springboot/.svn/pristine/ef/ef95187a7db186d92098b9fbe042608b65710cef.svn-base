 ...
 */
public class FrontLoginContextInterceptor extends HandlerInterceptorAdapter {

	private Logger logger = LoggerFactory.getLogger(FrontLoginContextInterceptor.class);

	public FrontLoginContextInterceptor() {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		String sessionId = CookieUtils.getCookie(request, "frontSessionId"); // request.getHeader("sessionId");
		String userId = RedisUtil.get(ConstantsUtils.SESSION_KEY_PREFIX_USER + sessionId);
		String venueUserId = RedisUtil.get(ConstantsUtils.SESSION_KEY_PREFIX_FRONT_VENUE + sessionId);
		// 是否需要拦截
		boolean flag = true;
		if (userId == null && venueUserId == null) {
			...
			response.sendRedirect(envProperties.getServerDynamic()+"/front");
			return false;
		}
		...
		VenueUserEntity venueUserEntity = RedisUtil.get(ConstantsUtils.SESSION_KEY_PREFIX_FRONT_VENUE_ENTITY + sessionId);
