...
public class VenueUserController extends BaseController<VenueUserEntity, VenueUserService> {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/toMovementLike")
	public String toMovementLike (String titleofLast, HttpServletRequest request) {
		request.setAttribute("titleofLast", titleofLast);
		return "/system/my/movement_like";
	}
	...
	@ResponseBody
	public Map<String, Object> saveRegister(String mobileCode, String msgCode, String passwordCode, String verifyCodeImgId) {
...
//	//钱包地址_处理历史数据
//	if (blockChainProperties.getEnabled()) {
//		List<VenueUserEntity> list=service.findAll();
//		for (VenueUserEntity entity1 : list) {
//			String address = service.getAddress(entity1.getId());
//			BlockChainUtil.sendData(address, "FCMS", 10, "register");
//			service.upadateUserTotal(10L, entity1.getId());
//		}
//	}
	...
	public Map<String, Object> saveUserInfo(String uuid, String name, Integer sex, String mobile, String email,
		...
		if(SensitiveWordsUtil.sensitiveFlag(experience)) {
			result.put("success", false);
			result.put("msg", "您的个性签名内容不符合规范！");
			return result;
		}
		
		/*if(SensitiveWordsUtil.sensitiveFlag(email)) {
			result.put("success", false);
			result.put("msg", "您的邮箱不符合规范！");
			return result;
		}*/
