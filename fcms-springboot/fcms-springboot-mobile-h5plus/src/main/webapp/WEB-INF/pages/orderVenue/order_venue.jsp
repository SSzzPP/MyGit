		...
		<script>
					...
					function (data) {
						...
						if(data.total != 0) {
							mui.each(data.rows,function (index,item) {
								var status = '';
								if(item.cancleStatus == null){
									if(item.status ==0)
										status = '未接单';
									else
										status = '已接单';
								}else{
									if(item.cancleStatus ==0)
										status = '退订中';
									else
										status = '已退订';
								}
								str += '<li class="mui-table-view-cell" id="'+item.uuid+'">';
								...
								str += '	</div>';
								str += '	<div class="siteInfo mui-text-right">';
								if(item.cancleStatus != null){
									str += '	<span class="label">退订</span>';
								}
								if(item.status == 1){
									str += '	<span class="label">已接单</span>';
								}else if(item.editStatus != null){
									str += '	<span class="label">改签</span>';
								}
								str += '	</div>';
								str += '</li>';
							});
			...
			function toDetail () {
				$("#orderList li").on('tap',function() {
					...
					mui.openWindow({
						 url: dynamicURL+'mobile/order/toVenueOrderDetail?uuid='+uuid, 
						 id : 'mobile/order/toVenueOrderDetail',
					    show:{
