	...
	<body>
		<header class="mui-bar mui-bar-nav" style="text-align:left;background-color:#fff;box-shadow:none;">
			<div class="mui-input-row">
				...
				<input id="serachInput"  name="search_LIKE_name" class="selinput mui-input-clear" type="text" placeholder="请输入健身路径名"/>
		...
		<script>
			...
			/*加载场馆*/
				function loadPubEquip(street,pageRows,pageNum,pubEquipName) {
					if (pageNum == 1) {
						$("#pubEquipList").empty();
					}
					var params = {
						...
						function (data) {
							...
							if(data.total != 0) {
								mui.each(data.rows,function (index,item) {
									...
									str += '		<div class="con">所属街道：<span>'+(item.businessHours == null ? '暂未设置' : item.streetName)+'</span></div>';
			...
			/*显示  隐藏筛选条件*/
				...
				function toDetail () {
					$("#pubEquipList li").on('tap',function() {
