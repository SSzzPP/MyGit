	...
	<body>
		...
		<div class="mui-content">
			<form action="javascript:void(0)">
				<div class="mui-input-row">
					...
					<input id='coachId' type="hidden" class="" >
					<input id='courseId' type="hidden" class="" >
				</div>
				<div class="mui-input-row">
					<label>手机号码</label>
				...
				<div class="mui-input-row">
					<label>教练姓名</label>
					<input id='coachName' type="text" class="" disabled="true">
				</div>
				<div class="mui-input-row">
					<label>课程名称</label>
					<input id='courseName' type="text" class="" disabled="true">
				</div>
				<div class="mui-input-row">
					<label>预约开始时间</label>
				...
				<div class="mui-input-row">
					<label>预约课时</label>
					<input id='count' type="number" class="" placeholder="请输入预约课时">
				</div>
		...
		<script>
			...
			mui.ready(function(){
			    ...
			    //coachId =  plus.webview.currentWebview().userId;
			    //courseId = plus.webview.currentWebview().courseId;
			    //orderUuid = plus.webview.currentWebview().orderUuid;
			    //type = plus.webview.currentWebview().type;
				$("#count").val(1);
			    topUtil.web_query('/mobile/venue/course/detail', {id:courseId}, function (data) {
					if (typeof data == "string") {
		                data = JSON.parse(data);
		            }
					if(data.success) {
						$("#coachName").val(data.obj.venueUserName);
		        		$("#courseName").val(data.obj.name);
					}else{
						mui.alert("功能异常，请联系客服！");
					}
				});
			    if(orderUuid){
