...
(function($, doc) {
	...
	var canvasState = doc.getElementById('canvasState');// canvas使用状态 0:启用；1禁用
...
//				cxt.lineWidth = 2 * 3;
		...
		isPointSelect: function(touches, linePoint) {
			// 有 canvasState 但是禁用,则return
			if(canvasState && canvasState.value==1) return;
		...
		initEvent: function(canvas, cxt, holder) {
			...
			self._endHandler = function(e) {
				//有 canvasState 但是禁用,则return
				if(canvasState && canvasState.value==1) {
