...
<script type="text/javascript" src="https://api.map.baidu.com/api?v=2.0&ak=3sCbOcosKfB6d3f47GtHYNjXVYdojnzc"></script>
<script type="text/javascript" src="${staticURL }/js/LuShu_min.js"></script>
		...
		<div data-options="region:'north',split:false,collapsible:false">
			<div class="card-button">
				<div class="layui-btn-container">
					<button id="run" class="layui-btn layui-btn">开始</button> 
					<button id="stop" class="layui-btn layui-btn">停止</button> 
					<button id="pause" class="layui-btn layui-btn">暂停</button> 
				</div>
			</div>
		</div>
		<div data-options="region:'center',split:false,collapsible:false" style="width: 100%;padding:10px 15px 10px 5px;">
...
<script type="text/javascript">
	...
	function initStyleAndArea() {
		...
		jQuery.ajax({
			type : "POST",
			url : "${dynamicURL}/admin/map/area/detail?id=${areaId}",
			dataType:"json",
			async:false,
			success : function(result) {
				if(result.success){
					//将先前绘制的区域显示在地图上
					var points = result.obj.points.split("_");
					...
					map.addOverlay(ply); //添加覆盖物 
					//将先前绘制的线显示在地图上
					if(result.obj.linePoints){
						var linePoints = result.obj.linePoints.split("_");
	...
	function getLine() {
		...
		jQuery.ajax({
			type : "POST",
			url : "${dynamicURL}/admin/checklistbillgps/noPageList?search_EQ_billId=${id}&sort=gmtCreate&order=asc",
			dataType:"json",
			async:false,
			success : function(result) {
				if(result.length){
					...
					jQuery.each(result,function(index,data){
						var bp=new BMap.Point(data.lng, data.lat);
						//arrPois.push(bp);
						arrPois= arrPois.concat(bp);
						//getAllPoint(pointA,bp);
					});
					var ply = new BMap.Polyline(arrPois,polylineStyleOptions2); //建立多边形覆盖物
					map.addOverlay(ply); //添加覆盖物 
					//console.log(arrPois);
				}else{
					parent.layer.alert("获取巡查路线失败！", {icon: 0});
				}
			},
			error : function() {
			}
	...
	function getAllPoint(pointA,pointB){
		if(pointA != '' && pointB != ''){
			var ax = pointA.lng;
			var ay = pointA.lat;
			var bx = pointB.lng;
			...
			for(var i=1;i<=n;i++){
				...
				//arrPois.push(new_point);
				arrPois= arrPois.concat(new_point);
			}
		}
		return;
	}
