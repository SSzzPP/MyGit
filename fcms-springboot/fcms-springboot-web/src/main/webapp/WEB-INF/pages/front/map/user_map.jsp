...
<div class="container">
	                			...
	                			<tr>
									...
									<td>
										<div class="BtnLine" style="border: 0px; margin: 0px; padding: 0px;text-align:left">
											...
											<button type="button"  onclick="refresh()">重置</button>
										</div>
									</td>
	                			</tr>
	               			</table>
	...
	<script type="text/javascript">
		...
		function changeInfoWindow(){
			for ( var i = 0; i < markerArr.length; i++) {
				markerArr[i].addEventListener("click", function(e) {
					...
					var a ="<a href='${dynamicURL}/front/map/toMonitor' target='_blank'>实时监控</a> <span style='color:green;text-decoration:underline;'>开启</span> <a href='${dynamicURL}/front/map/toPanoramicMap?lng="+e.point.lng+"&lat="+e.point.lat+"' target='_blank'>进入全景>></a>";
					content = content + a;
		...
		function getGymnasium(){
			var url = '${dynamicURL}/front/map/venue/search?lng='+userPoint.lng+'&lat='+userPoint.lat+'&radius='+radius;
						...
						for(var  i=0;i<result.length;i++){
							(function(){
							    ...
							    if(gymnasium.mainImageUuid != "" && gymnasium.mainImageUuid != null){
							    	filePath = "${dynamicURL}/front/map/showImage?uuid="+gymnasium.mainImageUuid;
								...
								content += '<table cellspacing="0" style="overflow:hidden;table-layout:fixed;width:100%;font:12px arial,sans-serif" >'+
											...
											'<td>'+(gymnasium.location == null ? '暂未设置' : gymnasium.location)+'&nbsp;</td>'+
											'</tr>'+
											'<tr>'+
											'<td style="vertical-align:top;width:38px;white-space:nowrap;word-break:keep-all"">电话：&nbsp;</td>'+
		...
		function getEquipment(){
			var url = '${dynamicURL}/front/map/equipment/search?lng='+userPoint.lng+'&lat='+userPoint.lat+'&radius='+radius;
			if($("#typeFormId").val() != "") {
				url += '&search_EQ_typeId='+$("#typeFormId").val();
			}
			jQuery.ajax({
		...
		$(function() {  
			$.ajax({
				type : "POST",
				url : '${dynamicURL}/front/dict/combobox?type=BUSINESS_AREA',
				dataType : "JSON",
			...
			$.ajax({
				type : "POST",
				url : '${dynamicURL}/front/dict/combobox?type=SPORT_TYPE',
				dataType : "JSON",
				success : function(msg){
		...
		function showPubEquipInfo(id){
			window.location.href='${dynamicURL}/front/toEquipmentDetails?equipmentId='+id;
		}
	</script>
</body>
</body>
