 ...
 --%>	
	...
	<script>
		...
		function initInfo(pageNum){
			...
			var url = '';
			params.order = 'desc';
			params.search_EQ_status = 1
			url = '${dynamicURL}/front/survey/paper/list';
			$.ajax({
				url:url,
				data:params,
				type:"post",
				...
				success:function(result){
					if(result){
						...
						$.each(result.rows,function(i,v){
							var datetime = new Date(v.startDate);
							var endDatetime = new Date(v.endDate);
							var dateStr = myformatter(datetime);
							var now = new Date();
							//验证当前问卷是否结束
							var finished = dateCompare(endDatetime.toString(),now.toString());
							var str = '<li onclick="details('+v.id+','+finished+')">'+
										'<p class="link title">';
										if(finished)
											str+='<a class="articleid">'+v.title+'(已结束)</a>';
										else
											str+='<a class="articleid">'+v.title+'</a>';
										str+='</p>'+
										'<p class="time"><span class="wp-new-ar-pro-time">'+dateStr+'</span></p>'+
									'</li>';
							$("#info").append(str);
		...
		function details(id,finished){
			var now = new Date();
			if(finished){
				layer.alert("该问卷调查已结束!");
				return;
			}
        	window.location.href='${dynamicURL}/front/survey/paper/toPaperDetail?id='+id;
		}
		//分页的回调
