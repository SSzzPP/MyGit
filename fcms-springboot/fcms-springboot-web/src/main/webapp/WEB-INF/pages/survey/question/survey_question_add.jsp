...
<div id="container">
	<form class="editInfoFrm" method="post"><!-- action="${dynamicURL}/admin/survey/question/save"  -->
...
/* 	CKEDITOR.replace('titleFormId', {
 ...
 /* 	$("#container form.editInfoFrm").form({
    ...
    $("#container .editInfoFrm .BtnLine .submit").on("click", _.debounce(function() {
    	...
    	if (!questionType) {
    		layer.alert('题目类型不能为空！', 'error');
			return;
    	}
    	var title = $("[name=title]").val();
    	if (!title) {
    		layer.alert('题目标题不能为空！', 'error');
    		return;
    	}
    	var options = $("[name=options]").val();
    	if (!options) {
    		layer.alert('选项内容不能为空！', 'error');
    		return;
    	}
