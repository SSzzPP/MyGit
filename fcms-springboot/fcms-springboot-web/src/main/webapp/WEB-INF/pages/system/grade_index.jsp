...
$(function() {
    ...
    gradeDatagrid = $('#gradeDatagrid').datagrid({
        url: '${dynamicURL}/admin/system/grade/list',
...
function add() {
	parent._layer({
	    url: '${dynamicURL}/admin/system/grade/add',
	    title: "新增年级",
	    width:'600px',
	    height:'600px',
	    btn: ['保存', '取消']
	},
	{
	    yes: function(index, layero) {
	        ...
	        var frm = parent.$("#layui-layer-iframe" + parent.ixs["新增年级"]).contents().find("form.addForm");
	        $.submit({
	            url: "${dynamicURL}/admin/system/grade/save",
	            formTg:frm,
	            butObj : bObj,
	            cache: false,
	            success : function(res){
	            	if(res.success){
	            		$('#gradeDatagrid').datagrid('reload');
	    	         	...
	    	         	parent.layer.close(parent.parent.ixs["新增年级"]);
	            	}else{
...
/*删除*/
function del() {
    var row = gradeDatagrid.datagrid("getSelected");
    if (row) {  	
    	//删除询问框
    	layer.confirm('是否删除？', {
    	  btn: ['确定','取消'] //按钮
    		...
    		$.submit({
                url: "${dynamicURL}/admin/system/grade/deleteInfo",
                data:{
                    "ids": row.id
                },
                butObj : null,
...
function edit() {
	...
	if(row){
		parent._layer({
	        url: '${dynamicURL}/admin/system/grade/add?id='+row.id,
	        title: "修改班级",
	        width:'600px',
	        height:'600px',
	        btn: ['保存', '取消']
	    },
	    {
	        yes: function(index, layero) {
	        	var bObj = layero.find("a.layui-layer-btn0");
	        	var frm = parent.$("#layui-layer-iframe" + parent.ixs["修改年级"]).contents().find("form.addForm");
	           /*  	
	        	...
	        	$.submit({
		            ...
		            success : function(res){
		            	if(res.success){
			            	$('#gradeDatagrid').datagrid('reload');
			            	layer.alert('保存成功！', {icon: 1});
			            	parent.layer.close(parent.parent.ixs["修改年级"]);
		            	}else{
